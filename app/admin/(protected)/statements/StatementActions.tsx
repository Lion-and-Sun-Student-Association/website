...
import {
  submitStatement,
  publishStatement,
  requestStatementChanges,
} from "./actions";

const btn = "rounded-lg px-3 py-1.5 text-sm font-medium disabled:opacity-60";

/**
 * Role- and status-aware review controls for a statement.
 *  - Execs submit drafts / revisions for review.
 *  - Reviewers (president/owner) publish directly, or approve / request changes.
 */
  ...
  return (
    <div className="flex items-center gap-2">
      {submittable && !canReview && (
        <button
          disabled={pending}
          onClick={() => start(() => submitStatement(id))}
      ...
      {canReview && submittable && (
        <button
          disabled={pending}
          onClick={() => start(() => publishStatement(id))}
        ...
        <>
          ...
          <button
            ...
            onClick={() => {
              ...
              if (note) start(() => requestStatementChanges(id, note));
            }}
